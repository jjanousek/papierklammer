...
description: >
  Create new Papierklammer plugins with the current alpha SDK/runtime. Use when
...
# Create a Papierklammer Plugin

Use this skill when the task is to create, scaffold, or document a Papierklammer plugin.
...
```bash
pnpm --filter @papierklammer/create-papierklammer-plugin build
node packages/plugins/create-papierklammer-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Papierklammer repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.papierklammer-sdk/`:

```bash
pnpm --filter @papierklammer/create-papierklammer-plugin build
node packages/plugins/create-papierklammer-plugin/dist/index.js @acme/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/papierklammer/packages/plugins/sdk
```
...
- is installed into Papierklammer from an absolute local path during development
