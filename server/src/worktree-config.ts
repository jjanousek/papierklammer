...
import type { PapierklammerConfig } from "@papierklammer/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    "# Papierklammer environment variables",
    "# Generated by Papierklammer worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.PAPIERKLAMMER_IN_WORKTREE !== "true") return null;

  const configPath = resolvePapierklammerConfigPath(overrideConfigPath);
  const envPath = resolvePapierklammerEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.PAPIERKLAMMER_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.PAPIERKLAMMER_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
...
function writeConfigFile(configPath: string, config: PapierklammerConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as PapierklammerConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: PapierklammerConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: PapierklammerConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.PAPIERKLAMMER_HOME = context.homeDir;
  process.env.PAPIERKLAMMER_INSTANCE_ID = context.instanceId;
  process.env.PAPIERKLAMMER_CONFIG = context.configPath;
  process.env.PAPIERKLAMMER_CONTEXT = context.contextPath;
  process.env.PAPIERKLAMMER_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as PapierklammerConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    PAPIERKLAMMER_HOME: context.homeDir,
    PAPIERKLAMMER_INSTANCE_ID: context.instanceId,
    PAPIERKLAMMER_CONFIG: context.configPath,
    PAPIERKLAMMER_CONTEXT: context.contextPath,
    PAPIERKLAMMER_IN_WORKTREE: "true",
    PAPIERKLAMMER_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: PapierklammerConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as PapierklammerConfig;
  } catch {
    return;
  }
