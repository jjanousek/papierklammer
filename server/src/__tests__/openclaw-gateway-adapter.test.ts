...
import { execute, testEnvironment } from "@papierklammer/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@papierklammer/adapter-openclaw-gateway/ui";
import type { AdapterExecutionContext } from "@papierklammer/adapter-utils";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("papierklammer:issue:issue-123");
      expect(payload?.papierklammer).toMatchObject({
        runId: "run-123",
        companyId: "company-123",
        agentId: "agent-123",
        taskId: "task-123",
      ...
      expect(String(payload?.message ?? "")).toContain("PAPIERKLAMMER_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("PAPIERKLAMMER_TASK_ID=task-123");
