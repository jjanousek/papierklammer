...
import { testEnvironment } from "@papierklammer/adapter-gemini-local/server";
...
const outPath = process.env.PAPIERKLAMMER_TEST_ARGS_PATH;
if (outPath) {
    ...
    const result = await testEnvironment({
      ...
      config: {
        ...
        env: {
          ...
          PAPIERKLAMMER_TEST_ARGS_PATH: argsCapturePath,
