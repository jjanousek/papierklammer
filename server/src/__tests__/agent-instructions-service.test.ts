...
describe("agent instructions service", () => {
  const originalPaperclipHome = process.env.PAPIERKLAMMER_HOME;
  const originalPaperclipInstanceId = process.env.PAPIERKLAMMER_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalPaperclipHome === undefined) delete process.env.PAPIERKLAMMER_HOME;
    else process.env.PAPIERKLAMMER_HOME = originalPaperclipHome;
    if (originalPaperclipInstanceId === undefined) delete process.env.PAPIERKLAMMER_INSTANCE_ID;
    else process.env.PAPIERKLAMMER_INSTANCE_ID = originalPaperclipInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    ...
    process.env.PAPIERKLAMMER_INSTANCE_ID = "test-instance";
