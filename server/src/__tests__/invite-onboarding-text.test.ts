...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Papierklammer OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Papierklammer base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("papierklammerApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.papierklammerApiUrl");
    expect(text).toContain("~/.openclaw/workspace/papierklammer-claimed-api-key.json");
    expect(text).toContain("PAPIERKLAMMER_API_KEY");
