...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@papierklammer/db";
 ...
 *
 * This implementation delegates to the core Papierklammer domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@papierklammer/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
