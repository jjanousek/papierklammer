 ...
 */
import { pluginManifestV1Schema } from "@papierklammer/shared";
import type { PapierklammerPluginManifestV1 } from "@papierklammer/shared";
import { PLUGIN_API_VERSION } from "@papierklammer/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): PapierklammerPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as PapierklammerPluginManifestV1,
        };
      }
