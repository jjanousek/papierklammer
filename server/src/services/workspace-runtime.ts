...
import type { AdapterRuntimeServiceReport } from "@papierklammer/adapter-utils";
import type { Db } from "@papierklammer/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@papierklammer/db";
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".papierklammer", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Papierklammer", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
