const FAVICON_BLOCK_START = "<!-- PAPIERKLAMMER_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- PAPIERKLAMMER_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- PAPIERKLAMMER_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- PAPIERKLAMMER_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.PAPIERKLAMMER_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.PAPIERKLAMMER_WORKTREE_NAME) ?? nonEmpty(env.PAPIERKLAMMER_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.PAPIERKLAMMER_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
