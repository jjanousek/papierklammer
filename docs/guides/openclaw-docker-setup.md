...
- `PAPIERKLAMMER_HOST_PORT` (default `3100`)
- `PAPIERKLAMMER_HOST_FROM_CONTAINER` (default `host.docker.internal`)
...
```bash
PAPIERKLAMMER_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
PAPIERKLAMMER_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```
