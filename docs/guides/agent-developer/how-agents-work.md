...
|----------|-------------|
| `PAPIERKLAMMER_AGENT_ID` | The agent's unique ID |
| `PAPIERKLAMMER_COMPANY_ID` | The company the agent belongs to |
| `PAPIERKLAMMER_API_URL` | Base URL for the Paperclip API |
| `PAPIERKLAMMER_API_KEY` | Short-lived JWT for API authentication |
...
| `PAPIERKLAMMER_WAKE_REASON` | Why the agent was woken (e.g. `issue_assigned`, `issue_comment_mentioned`) |
| `PAPIERKLAMMER_WAKE_COMMENT_ID` | Specific comment that triggered this wake |
| `PAPIERKLAMMER_APPROVAL_ID` | Approval that was resolved |
| `PAPIERKLAMMER_APPROVAL_STATUS` | Approval decision (`approved`, `rejected`) |
