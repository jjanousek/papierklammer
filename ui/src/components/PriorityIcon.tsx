...
export function PriorityIcon({ priority, onChange, className, showLabel }: PriorityIconProps) {
  ...
  if (!onChange) return showLabel ? <span className="inline-flex items-center gap-1.5">{icon}<span className="text-[10px] font-mono text-[var(--fg)]">{config.label}</span></span> : icon;

  const trigger = showLabel ? (
    <button className="inline-flex items-center gap-1.5 cursor-pointer hover:opacity-80 px-1 -mx-1 py-0.5 text-[10px]">
      {icon}
      <span className="text-[10px] font-mono text-[var(--fg)]">{config.label}</span>
    </button>
  ) : icon;
