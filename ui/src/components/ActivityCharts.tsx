    ...
    <div className="flex flex-wrap gap-x-2.5 gap-y-0.5 mt-2">
      {items.map(item => (
        <span key={item.label} className="flex items-center gap-1 text-[9px] text-muted-foreground">
          <span className="h-1.5 w-1.5 shrink-0" style={{ backgroundColor: item.color }} />
          {item.label}
        </span>
      ))}
    </div>
...
export function ChartCard({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) {
  return (
    <div className="border border-border p-4 space-y-3">
      <div>
        <h3 className="text-[9px] uppercase tracking-[1px] text-[var(--fg-dim)]">{title}</h3>
        {subtitle && <span className="text-[10px] text-[var(--fg-dim)]">{subtitle}</span>}
      </div>
      {children}
    </div>
  );
            ...
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={`${day}: ${total} runs`}>
              {total > 0 ? (
                <div className="flex flex-col-reverse gap-px overflow-hidden" style={{ height: `${heightPct}%`, minHeight: 2 }}>
                  {entry.succeeded > 0 && <div className="bg-[var(--alive)]" style={{ flex: entry.succeeded }} />}
                  {entry.failed > 0 && <div className="bg-[var(--dead)]" style={{ flex: entry.failed }} />}
                  {entry.other > 0 && <div className="bg-[var(--fg-muted)]" style={{ flex: entry.other }} />}
                </div>
              ) : (
