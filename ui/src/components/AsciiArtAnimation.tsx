...
type PapierklammerSprite = (typeof PAPIERKLAMMER_SPRITES)[number];
...
function spriteSize(sprite: PapierklammerSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = PAPIERKLAMMER_SPRITES[(baseCol + baseRow) % PAPIERKLAMMER_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = PAPIERKLAMMER_SPRITES[Math.floor(Math.random() * PAPIERKLAMMER_SPRITES.length)]!;
