...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`papierklammer-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="papierklammer-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("papierklammer-mermaid-status", error && "papierklammer-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="papierklammer-mermaid-source">
      ...
      if (parsed) {
        ...
        return (
          <a
            href={targetHref}
            className={cn(
              "papierklammer-mention-chip",
              `papierklammer-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "papierklammer-project-mention-chip",
            )}
