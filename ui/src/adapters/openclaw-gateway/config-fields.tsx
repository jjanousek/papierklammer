    ...
    <Field label={label}>
      <div className="relative">
        <button
          ...
          className="absolute left-2 top-1/2 -translate-y-1/2 text-muted-foreground/50 hover:text-muted-foreground"
        >
...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Papierklammer API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "papierklammerApiUrl",
                  String(config.papierklammerApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "papierklammerApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://papierklammer.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "papierklammer"))}
                onCommit={(v) => mark("adapterConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="papierklammer"
              />
            </Field>
          )}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Papierklammer persists a device key during onboarding so pairing approvals
