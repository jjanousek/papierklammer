...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("papierklammer-worktree-enabled") !== "true") return null;

  const name = readMetaContent("papierklammer-worktree-name");
  const color = normalizeHexColor(readMetaContent("papierklammer-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("papierklammer-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
