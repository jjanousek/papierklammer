...
import type { PapierklammerPluginManifestV1 } from "@papierklammer/shared";

import type { PapierklammerPlugin } from "./define-plugin.js";
import type {
 ...
 * import { startWorkerRpcHost } from "@papierklammer/plugin-sdk";
 *
 * startWorkerRpcHost({ plugin });
 * ```
 *
...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
  ...
  let manifest: PapierklammerPluginManifestV1 | null = null;
  ...
  const toolHandlers = new Map<string, {
    declaration: Pick<import("@papierklammer/shared").PluginToolDeclaration, "displayName" | "description" | "parametersSchema">;
