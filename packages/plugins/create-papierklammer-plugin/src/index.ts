...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".papierklammer-sdk");
...
/**
 * Generate a complete Papierklammer plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Papierklammer plugin";
  ...
  const packageJson = {
    ...
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@papierklammer/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@papierklammer/plugin-sdk": sdkDependency,
...
import { createPluginBundlerPresets } from "@papierklammer/plugin-sdk/bundlers";
...
`,
  ...
  writeFile(
    ...
    `import type { PapierklammerPluginManifestV1 } from "@papierklammer/plugin-sdk";

const manifest: PapierklammerPluginManifestV1 = {
  id: ${quote(manifestId)},
  apiVersion: 1,
  version: "0.1.0",
...
`,
  ...
  writeFile(
    ...
    `import { definePlugin, runWorker } from "@papierklammer/plugin-sdk";
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@papierklammer/plugin-sdk/ui";
...
import { createTestHarness } from "@papierklammer/plugin-sdk/testing";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@papierklammer/plugin-sdk\` and \`@papierklammer/shared\` from a local Papierklammer checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.papierklammer-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}

## Install Into Papierklammer
...
- \`pnpm build\` uses esbuild presets from \`@papierklammer/plugin-sdk/bundlers\`.
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-papierklammer-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <papierklammer-sdk-path>]");
    process.exit(1);
  }
