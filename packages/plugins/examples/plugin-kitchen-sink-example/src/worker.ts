...
import type { Goal, Issue } from "@papierklammer/shared";
import {
  DEFAULT_CONFIG,
  JOB_KEYS,
  PLUGIN_ID,
...
async function registerToolHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.tools.register(
    TOOL_NAMES.companySummary,
    {
      ...
      description: "Summarizes current company counts from the Papierklammer APIs.",
...
const plugin: PapierklammerPlugin = definePlugin({
  async setup(ctx) {
    currentContext = ctx;
