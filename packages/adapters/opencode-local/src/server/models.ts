...
function resolveOpenCodeCommand(input: unknown): string {
  const envOverride =
    typeof process.env.PAPIERKLAMMER_OPENCODE_COMMAND === "string" &&
    process.env.PAPIERKLAMMER_OPENCODE_COMMAND.trim().length > 0
      ? process.env.PAPIERKLAMMER_OPENCODE_COMMAND.trim()
      : "opencode";
