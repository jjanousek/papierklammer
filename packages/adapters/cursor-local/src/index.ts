...
Use when:
- You want Papierklammer to run Cursor Agent CLI locally as the agent runtime
...
- Papierklammer auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$papierklammer" and related skills on local runs.
- Papierklammer auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
