...
const DEFAULT_PAPIERKLAMMER_INSTANCE_ID = "default";
...
function isWorktreeMode(env: NodeJS.ProcessEnv): boolean {
  return TRUTHY_ENV_RE.test(env.PAPIERKLAMMER_IN_WORKTREE ?? "");
}
...
export async function prepareManagedCodexHome(
  ...
  await onLog(
    "stdout",
    `[papierklammer] Using ${isWorktreeMode(env) ? "worktree-isolated" : "Papierklammer-managed"} Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
  );
  return targetHome;
}
