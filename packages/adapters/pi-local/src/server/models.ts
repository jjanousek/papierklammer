...
import type { AdapterModel } from "@papierklammer/adapter-utils";
import { asString, runChildProcess } from "@papierklammer/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.PAPIERKLAMMER_PI_COMMAND === "string" &&
    process.env.PAPIERKLAMMER_PI_COMMAND.trim().length > 0
      ? process.env.PAPIERKLAMMER_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["PAPIERKLAMMER_", "npm_", "NPM_"] as const;
