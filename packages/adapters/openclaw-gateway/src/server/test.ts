...
import { asString, parseObject } from "@papierklammer/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Papierklammer server host.",
        });
      }
    } catch (err) {
      checks.push({
