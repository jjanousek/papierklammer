...
describe("transcript secret redaction", () => {
  ...
  it("redacts contextual secret values across command output and settled transcript text", () => {
    const messages: ChatMessage[] = [
      {
        role: "assistant",
        text: RAW_CONTEXTUAL_SECRET,
        timestamp: new Date(),
        blocks: [
          {
            type: "command",
            item: {
              command: `env OPENAI_API_KEY=${RAW_CONTEXTUAL_SECRET} printenv OPENAI_API_KEY`,
              output: RAW_CONTEXTUAL_SECRET,
              status: "completed",
              exitCode: 0,
            },
          },
    ...
    const frame = lastFrame() ?? "";
    expect(frame).toContain("OPENAI_API_KEY=");
    expect(frame).not.toContain(RAW_CONTEXTUAL_SECRET);
    expect(frame).toMatch(/redacted len=/i);
  ...
  it("keeps contextual placeholder secrets redacted in live and finalized transcript surfaces", async () => {
    const mockProc = createMockProcess();
    const mockSpawn = vi.fn().mockReturnValue(mockProc);
    const mockFetch = createMockFetch();
    ...
    stdin.write("Inspect env placeholder");
    await tick();
    stdin.write("\r");
    await tick(100);

    respond(mockProc, { id: 1, result: { thread: { id: "thr_contextual_redact" } } });
    await tick();
    respond(mockProc, {
      id: 2,
      result: {
        turn: { id: "turn_contextual_redact", status: "inProgress", items: [], error: null },
      },
    });
    await tick();
    ...
    respond(mockProc, {
      method: "item/started",
      params: {
        threadId: "thr_contextual_redact",
        turnId: "turn_contextual_redact",
        item: {
          type: "commandExecution",
          id: "cmd_contextual_redact",
          command: `env OPENAI_API_KEY=${RAW_CONTEXTUAL_SECRET} printenv OPENAI_API_KEY`,
          cwd: "/tmp",
          aggregatedOutput: "",
          exitCode: null,
    ...
    respond(mockProc, {
      ...
      params: {
        threadId: "thr_contextual_redact",
        turnId: "turn_contextual_redact",
        itemId: "cmd_contextual_redact",
        delta: RAW_CONTEXTUAL_SECRET,
      },
    });
    await tick();
    ...
    const liveFrame = await waitForFrame(
      lastFrame,
      (frame) => frame.includes("OPENAI_API_KEY=") || frame.includes("redacted len="),
    );

    expect(liveFrame).not.toContain(RAW_CONTEXTUAL_SECRET);
    expect(liveFrame).toMatch(/redacted len=/i);
    ...
    respond(mockProc, {
      ...
      params: {
        ...
        item: {
          ...
          command: `env OPENAI_API_KEY=${RAW_CONTEXTUAL_SECRET} printenv OPENAI_API_KEY`,
          cwd: "/tmp",
          aggregatedOutput: RAW_CONTEXTUAL_SECRET,
          exitCode: 0,
          status: "completed",
        },
      },
    ...
    respond(mockProc, {
      ...
      params: {
        threadId: "thr_contextual_redact",
        turn: { id: "turn_contextual_redact", status: "completed", items: [], error: null },
      },
    });
    await tick(100);
