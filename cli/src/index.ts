...
const DATA_DIR_OPTION_HELP =
  "Papierklammer data directory root (isolates state from ~/.papierklammer)";

program
  .name("papierklammer")
  .description("Papierklammer CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadPapierklammerEnvFile(options.config);
});
...
program
  ...
  .option("--run", "Start Papierklammer immediately after saving config", false)
  .action(onboard);
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "papierklammer")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Papierklammer")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Papierklammer server API")
