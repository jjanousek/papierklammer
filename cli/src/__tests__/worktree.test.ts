...
describe("worktree helpers", () => {
  ...
  it("builds isolated config and env paths for a worktree", () => {
    ...
    expect(env.PAPIERKLAMMER_HOME).toBe(path.resolve("/tmp/paperclip-worktrees"));
    expect(env.PAPIERKLAMMER_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.PAPIERKLAMMER_IN_WORKTREE).toBe("true");
    expect(env.PAPIERKLAMMER_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.PAPIERKLAMMER_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export PAPIERKLAMMER_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    ...
    const originalInlineMasterKey = process.env.PAPIERKLAMMER_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.PAPIERKLAMMER_SECRETS_MASTER_KEY_FILE;
    try {
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.PAPIERKLAMMER_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.PAPIERKLAMMER_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.PAPIERKLAMMER_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          PAPIERKLAMMER_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    ...
    const originalJwtSecret = process.env.PAPIERKLAMMER_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.PAPIERKLAMMER_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".papierklammer-worktrees"),
      });

      const envPath = path.join(repoRoot, ".papierklammer", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("PAPIERKLAMMER_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("PAPIERKLAMMER_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/PAPIERKLAMMER_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.PAPIERKLAMMER_AGENT_JWT_SECRET;
      } else {
        process.env.PAPIERKLAMMER_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    ...
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".papierklammer-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".papierklammer", "config.json"), "utf8"));
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(worktreePath, ".papierklammer", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".papierklammer", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
