...
function createServerEnv(configPath: string, port: number, connectionString: string) {
  ...
  env.PAPIERKLAMMER_CONFIG = configPath;
  env.DATABASE_URL = connectionString;
  env.HOST = "127.0.0.1";
  env.PORT = String(port);
  env.SERVE_UI = "false";
  env.PAPIERKLAMMER_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.PAPIERKLAMMER_MIGRATION_AUTO_APPLY = "true";
  env.PAPIERKLAMMER_UI_DEV_MIDDLEWARE = "false";
...
function createCliEnv() {
  ...
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.PAPIERKLAMMER_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.PAPIERKLAMMER_MIGRATION_AUTO_APPLY;
  delete env.PAPIERKLAMMER_UI_DEV_MIDDLEWARE;
  return env;
}
...
async function runCliJson<T>(args: string[], opts: { apiBase: string; configPath: string }) {
  ...
  const result = await execFileAsync(
    "pnpm",
    ["--silent", "papierklammer", ...args, "--api-base", opts.apiBase, "--config", opts.configPath, "--json"],
    {
      cwd: repoRoot,
      env: createCliEnv(),
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `papierklammer run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("papierklammer company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  ...
  beforeAll(async () => {
    ...
    const child = spawn(
      "pnpm",
      ["papierklammer", "run", "--config", configPath],
      {
        cwd: repoRoot,
