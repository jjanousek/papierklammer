...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolvePapierklammerInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolvePapierklammerInstanceId());
}
...
export async function envCommand(opts: { config?: string }): Promise<void> {
  ...
  let config: PapierklammerConfig | null = null;
...
function collectDeploymentEnvRows(config: PapierklammerConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrl =
    process.env.PAPIERKLAMMER_PUBLIC_URL ??
    process.env.PAPIERKLAMMER_AUTH_PUBLIC_BASE_URL ??
  ...
  const publicUrlSource: EnvSource =
    process.env.PAPIERKLAMMER_PUBLIC_URL
      ? "env"
      : process.env.PAPIERKLAMMER_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
        ? "env"
        : config?.auth?.publicBaseUrl
          ? "config"
          : "missing";
  ...
  const secretsProvider =
    process.env.PAPIERKLAMMER_SECRETS_PROVIDER ??
    config?.secrets?.provider ??
    DEFAULT_SECRETS_PROVIDER;
  const secretsStrictMode =
    process.env.PAPIERKLAMMER_SECRETS_STRICT_MODE ??
    String(config?.secrets?.strictMode ?? false);
  const secretsKeyFilePath =
    process.env.PAPIERKLAMMER_SECRETS_MASTER_KEY_FILE ??
    config?.secrets?.localEncrypted?.keyFilePath ??
    defaultSecretsKeyFilePath();
  const storageProvider =
    process.env.PAPIERKLAMMER_STORAGE_PROVIDER ??
  ...
  const storageS3Endpoint =
    process.env.PAPIERKLAMMER_STORAGE_S3_ENDPOINT ??
    config?.storage?.s3?.endpoint ??
    "";
  const storageS3Prefix =
    process.env.PAPIERKLAMMER_STORAGE_S3_PREFIX ??
    config?.storage?.s3?.prefix ??
    "";
  const storageS3ForcePathStyle =
    process.env.PAPIERKLAMMER_STORAGE_S3_FORCE_PATH_STYLE ??
    String(config?.storage?.s3?.forcePathStyle ?? false);

  const rows: EnvVarRow[] = [
    {
      key: "PAPIERKLAMMER_AGENT_JWT_SECRET",
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from PAPIERKLAMMER_PUBLIC_URL when possible)",
    },
    {
      key: "PAPIERKLAMMER_AGENT_JWT_TTL_SECONDS",
      value: process.env.PAPIERKLAMMER_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.PAPIERKLAMMER_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "PAPIERKLAMMER_AGENT_JWT_ISSUER",
      value: process.env.PAPIERKLAMMER_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.PAPIERKLAMMER_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "PAPIERKLAMMER_AGENT_JWT_AUDIENCE",
      value: process.env.PAPIERKLAMMER_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.PAPIERKLAMMER_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "PAPIERKLAMMER_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.PAPIERKLAMMER_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "PAPIERKLAMMER_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.PAPIERKLAMMER_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "PAPIERKLAMMER_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.PAPIERKLAMMER_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "PAPIERKLAMMER_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.PAPIERKLAMMER_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "PAPIERKLAMMER_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.PAPIERKLAMMER_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "PAPIERKLAMMER_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.PAPIERKLAMMER_STORAGE_S3_BUCKET
        ? "env"
    ...
    {
      key: "PAPIERKLAMMER_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.PAPIERKLAMMER_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "PAPIERKLAMMER_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.PAPIERKLAMMER_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "PAPIERKLAMMER_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.PAPIERKLAMMER_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    {
      key: "PAPIERKLAMMER_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.PAPIERKLAMMER_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.PAPIERKLAMMER_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "PAPIERKLAMMER_CONFIG",
      value: process.env.PAPIERKLAMMER_CONFIG ?? configPath,
      source: process.env.PAPIERKLAMMER_CONFIG ? "env" : "default",
      required: false,
