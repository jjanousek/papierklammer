...
function buildAgentEnvExports(input: {
  ...
  return [
    `export PAPIERKLAMMER_API_URL='${escaped(input.apiBase)}'`,
    `export PAPIERKLAMMER_COMPANY_ID='${escaped(input.companyId)}'`,
    `export PAPIERKLAMMER_AGENT_ID='${escaped(input.agentId)}'`,
    `export PAPIERKLAMMER_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
