...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Papierklammer config file")
    .option("-d, --data-dir <path>", "Papierklammer data directory root (isolates state from ~/.papierklammer)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Papierklammer API")
...
export function resolveCommandContext(
  ...
  const companyId =
    ...
    process.env.PAPIERKLAMMER_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set PAPIERKLAMMER_COMPANY_ID, or set context profile companyId via `papierklammer context set`.",
    );
  }
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.PAPIERKLAMMER_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.PAPIERKLAMMER_SERVER_PORT || "");
