...
import { printPapierklammerCliBanner } from "../utils/banner.js";
...
type OnboardDefaults = Pick<PapierklammerConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "PAPIERKLAMMER_PUBLIC_URL",
  "DATABASE_URL",
  "PAPIERKLAMMER_DB_BACKUP_ENABLED",
  "PAPIERKLAMMER_DB_BACKUP_INTERVAL_MINUTES",
  "PAPIERKLAMMER_DB_BACKUP_RETENTION_DAYS",
  "PAPIERKLAMMER_DB_BACKUP_DIR",
  "PAPIERKLAMMER_DEPLOYMENT_MODE",
  "PAPIERKLAMMER_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "PAPIERKLAMMER_ALLOWED_HOSTNAMES",
  "PAPIERKLAMMER_AUTH_BASE_URL_MODE",
  "PAPIERKLAMMER_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "PAPIERKLAMMER_STORAGE_PROVIDER",
  "PAPIERKLAMMER_STORAGE_LOCAL_DIR",
  "PAPIERKLAMMER_STORAGE_S3_BUCKET",
  "PAPIERKLAMMER_STORAGE_S3_REGION",
  "PAPIERKLAMMER_STORAGE_S3_ENDPOINT",
  "PAPIERKLAMMER_STORAGE_S3_PREFIX",
  "PAPIERKLAMMER_STORAGE_S3_FORCE_PATH_STYLE",
  "PAPIERKLAMMER_SECRETS_PROVIDER",
  "PAPIERKLAMMER_SECRETS_STRICT_MODE",
  "PAPIERKLAMMER_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolvePapierklammerInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.PAPIERKLAMMER_PUBLIC_URL?.trim() ||
    process.env.PAPIERKLAMMER_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.PAPIERKLAMMER_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.PAPIERKLAMMER_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.PAPIERKLAMMER_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.PAPIERKLAMMER_ALLOWED_HOSTNAMES
    ? process.env.PAPIERKLAMMER_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.PAPIERKLAMMER_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.PAPIERKLAMMER_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.PAPIERKLAMMER_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.PAPIERKLAMMER_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.PAPIERKLAMMER_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.PAPIERKLAMMER_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.PAPIERKLAMMER_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.PAPIERKLAMMER_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.PAPIERKLAMMER_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.PAPIERKLAMMER_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.PAPIERKLAMMER_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.PAPIERKLAMMER_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.PAPIERKLAMMER_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.PAPIERKLAMMER_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.PAPIERKLAMMER_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "PAPIERKLAMMER_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<PapierklammerConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printPapierklammerCliBanner();
  p.intro(pc.bgCyan(pc.black(" papierklammer onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolvePapierklammerInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: PapierklammerConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Papierklammer install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("papierklammer configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("PAPIERKLAMMER_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.PAPIERKLAMMER_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("PAPIERKLAMMER_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("PAPIERKLAMMER_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: PAPIERKLAMMER_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("papierklammer run")}`,
        `Reconfigure later: ${pc.cyan("papierklammer configure")}`,
        `Diagnose setup: ${pc.cyan("papierklammer doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (!shouldRunNow && !opts.invokedByRun && process.stdin.isTTY && process.stdout.isTTY) {
      const answer = await p.confirm({
        message: "Start Papierklammer now?",
        initialValue: true,
      });
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.PAPIERKLAMMER_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing Papierklammer setup is ready.");
    return;
  }
  ...
  let llm: PapierklammerConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@papierklammer/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `papierklammer doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("papierklammer run")}`,
        `Then: ${pc.cyan("papierklammer auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
