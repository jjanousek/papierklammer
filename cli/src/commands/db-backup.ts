...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@papierklammer/db";
import {
  expandHomePrefix,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" papierklammer db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolvePapierklammerInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "papierklammer";
