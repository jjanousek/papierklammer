...
export const DEFAULT_WORKTREE_HOME = "~/.papierklammer-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".papierklammer");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: PapierklammerConfig | null;
...
export function buildWorktreeEnvEntries(
  ...
  return {
    PAPIERKLAMMER_HOME: paths.homeDir,
    PAPIERKLAMMER_INSTANCE_ID: paths.instanceId,
    PAPIERKLAMMER_CONFIG: paths.configPath,
    PAPIERKLAMMER_CONTEXT: paths.contextPath,
    PAPIERKLAMMER_IN_WORKTREE: "true",
    ...(branding?.name ? { PAPIERKLAMMER_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { PAPIERKLAMMER_WORKTREE_COLOR: branding.color } : {}),
  };
}
