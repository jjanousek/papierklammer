...
const JWT_SECRET_ENV_KEY = "PAPIERKLAMMER_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Papierklammer CLI commands",
...
export function resolvePapierklammerEnvFile(configPath?: string): string {
...
export function loadPapierklammerEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergePapierklammerEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readPapierklammerEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writePapierklammerEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergePapierklammerEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readPapierklammerEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writePapierklammerEnvEntries(next, filePath);
  return next;
}
