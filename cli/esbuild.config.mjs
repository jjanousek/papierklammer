/**
 * esbuild configuration for building the papierklammer CLI for npm.
 *
 * Bundles all workspace packages (@papierklammer/*) into a single file.
...
// to npm and resolved at runtime (e.g. @papierklammer/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@papierklammer/")) {
      externals.add(name);
    }
  }
