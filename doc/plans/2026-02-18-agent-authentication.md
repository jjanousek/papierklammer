...
`PAPIERKLAMMER_API_URL`, `PAPIERKLAMMER_AGENT_ID`, `PAPIERKLAMMER_COMPANY_ID`. We need to
add a `PAPIERKLAMMER_API_KEY` (JWT) to the set of injected env vars.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `PAPIERKLAMMER_API_KEY`.          |
