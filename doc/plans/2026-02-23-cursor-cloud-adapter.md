...
- `paperclipPublicUrl` (optional override; else `PAPIERKLAMMER_PUBLIC_URL` env)
...
Cursor agents run remotely, so we cannot inject local env like `PAPIERKLAMMER_API_KEY`.
...
2. `process.env.PAPIERKLAMMER_PUBLIC_URL`
