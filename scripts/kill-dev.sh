#!/usr/bin/env bash
#
# Kill all local Papierklammer dev server processes (across all worktrees).
#
# Usage:
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 ]]; then
  echo "No Papierklammer dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} Papierklammer dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to Papierklammer node processes..."
