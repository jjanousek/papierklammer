...
PAPIERKLAMMER_COMPANY_ID="${PAPIERKLAMMER_COMPANY_ID:-openclaw-smoke-company}"
PAPIERKLAMMER_API_URL="${PAPIERKLAMMER_API_URL:-http://localhost:3100}"
PAPIERKLAMMER_TASK_ID="${PAPIERKLAMMER_TASK_ID:-openclaw-smoke-task}"
PAPIERKLAMMER_WAKE_REASON="${PAPIERKLAMMER_WAKE_REASON:-openclaw_smoke_test}"
PAPIERKLAMMER_WAKE_COMMENT_ID="${PAPIERKLAMMER_WAKE_COMMENT_ID:-}"
PAPIERKLAMMER_APPROVAL_ID="${PAPIERKLAMMER_APPROVAL_ID:-}"
...
PAPIERKLAMMER_RUN_ID=${PAPIERKLAMMER_RUN_ID}
PAPIERKLAMMER_AGENT_ID=${PAPIERKLAMMER_AGENT_ID}
PAPIERKLAMMER_COMPANY_ID=${PAPIERKLAMMER_COMPANY_ID}
PAPIERKLAMMER_API_URL=${PAPIERKLAMMER_API_URL}
PAPIERKLAMMER_TASK_ID=${PAPIERKLAMMER_TASK_ID}
PAPIERKLAMMER_WAKE_REASON=${PAPIERKLAMMER_WAKE_REASON}
PAPIERKLAMMER_WAKE_COMMENT_ID=${PAPIERKLAMMER_WAKE_COMMENT_ID}
PAPIERKLAMMER_APPROVAL_ID=${PAPIERKLAMMER_APPROVAL_ID}
PAPIERKLAMMER_APPROVAL_STATUS=${PAPIERKLAMMER_APPROVAL_STATUS}
PAPIERKLAMMER_LINKED_ISSUE_IDS=${PAPIERKLAMMER_LINKED_ISSUE_IDS}
...
PAYLOAD="$(jq -nc \
  ...
  --arg runId "$PAPIERKLAMMER_RUN_ID" \
  --arg agentId "$PAPIERKLAMMER_AGENT_ID" \
  --arg companyId "$PAPIERKLAMMER_COMPANY_ID" \
  --arg apiUrl "$PAPIERKLAMMER_API_URL" \
  --arg taskId "$PAPIERKLAMMER_TASK_ID" \
  --arg wakeReason "$PAPIERKLAMMER_WAKE_REASON" \
  --arg wakeCommentId "$PAPIERKLAMMER_WAKE_COMMENT_ID" \
  --arg approvalId "$PAPIERKLAMMER_APPROVAL_ID" \
  --arg approvalStatus "$PAPIERKLAMMER_APPROVAL_STATUS" \
  --arg linkedIssueIds "$PAPIERKLAMMER_LINKED_ISSUE_IDS" \
  '{
    model: $model,
    user: $user,
    input: $text,
    stream: true,
    metadata: {
      PAPIERKLAMMER_RUN_ID: $runId,
      PAPIERKLAMMER_AGENT_ID: $agentId,
      PAPIERKLAMMER_COMPANY_ID: $companyId,
      PAPIERKLAMMER_API_URL: $apiUrl,
      PAPIERKLAMMER_TASK_ID: $taskId,
      PAPIERKLAMMER_WAKE_REASON: $wakeReason,
      PAPIERKLAMMER_WAKE_COMMENT_ID: $wakeCommentId,
