...
PAPIERKLAMMER_HOST_PORT="${PAPIERKLAMMER_HOST_PORT:-3100}"
PAPIERKLAMMER_HOST_FROM_CONTAINER="${PAPIERKLAMMER_HOST_FROM_CONTAINER:-host.docker.internal}"
...
detect_papierklammer_base_url() {
  ...
  for candidate in "$PAPIERKLAMMER_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${PAPIERKLAMMER_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${PAPIERKLAMMER_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
papierklammer_base_url="$(detect_papierklammer_base_url || true)"
...
Papierklammer URL for OpenClaw container:
EOF
  if [[ -n "$papierklammer_base_url" ]]; then
    cat <<EOF
  $papierklammer_base_url
...
EOF
  ...
  If Papierklammer rejects the host, run on host machine:
    pnpm papierklammer allowed-hostname host.docker.internal
  Then restart Papierklammer and re-run this script.
EOF
  fi
  cat <<EOF
