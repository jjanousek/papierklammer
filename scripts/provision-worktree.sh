...
async function main() {
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.PAPIERKLAMMER_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.PAPIERKLAMMER_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.PAPIERKLAMMER_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "PAPIERKLAMMER_HOME=" + JSON.stringify(worktreeHome),
    "PAPIERKLAMMER_INSTANCE_ID=" + JSON.stringify(instanceId),
    "PAPIERKLAMMER_CONFIG=" + JSON.stringify(configPath),
    "PAPIERKLAMMER_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "PAPIERKLAMMER_IN_WORKTREE=true",
    "PAPIERKLAMMER_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.PAPIERKLAMMER_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("PAPIERKLAMMER_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
